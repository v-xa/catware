...
configFilePath = f"{os.environ['LOCALAPPDATA']}\\temp\\catware"
    ...
    def esp(self):
        ...
        while pm.overlay_loop():
            ...
            if self.config["misc"]["watermark"]:
                watermark = f"catware | {pm.get_fps()}fps"
            ...
            if not self.config["esp"]["enabled"] and not self.config["misc"]["watermark"]:
                ...
                time.sleep(0.001) # prevent crashing overlay??? yes wtf
                ...
                if ent.wts(viewMatrix):
                    ...
                    if self.config["esp"]["skeleton"]:
                        try:
                            ...
                            # pm.draw_circle_lines(ent.headPos2d["x"], ent.headPos2d["y"], center / 3, Colors.white) # does it look bad?
...
if __name__ == "__main__":
    if os.name != "nt":
        input("catware is only working on Windows.")
    ...
    with dpg.window(label=title, width=uiWidth, height=uiHeight, no_collapse=True, no_move=True, no_resize=True, on_close=lambda: os._exit(0)) as window:
        with dpg.tab_bar():
            ...
            with dpg.tab(label="Settings"):
                ...
                creditsText = dpg.add_text(default_value="made by modulescripthash on discord!")
                githubText = dpg.add_text(default_value="https://discord.gg/romelon")
